...
test_size = 0.2
business_X_train,business_X_test, business_y_train, business_y_test = train_test_split(business['data'],business['res'], test_size=test_size)
entertainment_X_train,entertainment_X_test, entertainment_y_train, entertainment_y_test = train_test_split(entertainment['data'],entertainment['res'], test_size=test_size)
health_X_train, health_X_test, health_y_train, health_y_test = train_test_split(health['data'],health['res'], test_size=test_size)
politics_X_train, politics_X_test, politics_y_train, politics_y_test = train_test_split(politics['data'],politics['res'], test_size=test_size)
sports_X_train, sports_X_test, sports_y_train, sports_y_test = train_test_split(sports['data'],sports['res'], test_size=test_size)
technology_X_train, technology_X_test, technology_y_train, technology_y_test = train_test_split(technology['data'],technology['res'], test_size=test_size)
...
min_df = 2
C_param_range = [0.001,0.01,0.1,1,10,100, 150, 300]
temp_accuracy = []
avg_accuracies = []
for i in C_param_range:
    ...
    business_model = LogisticRegression(C = i)
    ...
    entertainment_model = LogisticRegression(C = i)
    ...
    health_model = LogisticRegression(C = i)
    ...
    politics_model = LogisticRegression(C = i)
    ...
    sports_model = LogisticRegression(C = i)
    ...
    technology_model = LogisticRegression(C = i)
    ...
    fn.print_pie(Accuracy)
    temp_accuracy.append(Accuracy)
    avg_accuracies.append(sum(temp_accuracy) / float(len(temp_accuracy)))
    temp_accuracy = []
    

fn.print_bars_for_acc(avg_accuracies)
...
predict_tweet = 'The presidential elections will held in this month'
prediction_table = []
